...
class DBPNS(nn.Module):
    def __init__(self, base_filter, feat, num_stages, scale_factor):
        ...
        self.feat1 = ConvBlock(
            base_filter, feat, 1, 1, 0, activation="prelu", norm=None
        )
        ...
        self.output = ConvBlock(
            num_stages * feat, feat, 1, 1, 0, activation=None, norm=None
        )
...
class RBPN(nn.Module):
    def __init__(
        ...
        self.feat0 = ConvBlock(
            num_channels, base_filter, 3, 1, 1, activation="prelu", norm=None
        )
        ...
        # Reconstruction
        self.output = ConvBlock(
            (nFrames - 1) * feat, num_channels, 3, 1, 1, activation=None, norm=None
        )
