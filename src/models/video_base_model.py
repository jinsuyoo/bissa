...
from src.models.sr_model import SRModel
from src.utils import imwrite, tensor2img
from src.utils.dist_util import get_dist_info

metric_module = importlib.import_module("src.metrics")
        ...
        for idx in range(rank, len(dataset), world_size):
            ...
            if save_img:
                if self.opt["is_train"]:
                    ...
                    if "vimeo" in dataset_name.lower():  # vimeo90k dataset
                        ...
                        img_name = (
                            f"{split_result[-3]}_{split_result[-2]}_"
            ...
            if with_metrics:
                ...
                for metric_idx, opt_ in enumerate(opt_metric.values()):
                    ...
                    result = getattr(metric_module, metric_type)(
                        result_img, gt_img, **opt_
                    )
            ...
            if rank == 0:
                for _ in range(world_size):
                    ...
                    pbar.set_description(
                        f"Test {folder}:" f"{int(frame_idx) + world_size}/{max_idx}"
                    )
        if rank == 0:
            pbar.close()
